...
def registrosAmostra():
    sql = "select * from amostras order by id"
    result = []
    try:
        for rows in cursor.execute(sql):
            result.append(rows)
...
def atualizarAmostra(mp10,mp25,o3,co,no2,so2,idAmostra):
    sql = f"UPDATE AMOSTRAS SET MP10={mp10},MP25={mp25},O3={o3},CO={co},NO2={no2},SO2={so2} WHERE ID= {idAmostra}"
    try:
        print(f"{sql}")
        cursor.execute(sql)
        cursor.execute("COMMIT")
...
def deletarAmostra(idAmostra):
    sql = f"DELETE FROM AMOSTRAS WHERE ID = {idAmostra}"
    try:
        print(f"{sql}")
        cursor.execute(sql)
        cursor.execute("COMMIT")
